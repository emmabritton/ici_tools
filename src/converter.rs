...
fn convert(image: Image) -> Result<IndexedImage> {
    ...
    if width >= 256 || height >= 256 {
        return Err(eyre!("Image is too big, max width and height are 255 (was {width} x {height})"));
    }

    if colors.len() >= 256 {
        return Err(eyre!("Image has too many colours, max is 255 (was {})", colors.len()));
    }

    let palette: Vec<IciColor> = colors.iter().map(|c| c.to_ici()).collect();
    let pixels = image.pixels().iter().map(|p| palette.iter().position(|c| c.to_color() == *p).unwrap() as u8).collect();

    Ok(IndexedImage::new(width as u8, height as u8, palette, pixels)?)
}

fn create_file_name(input: &PathBuf, output_file: Option<PathBuf>) -> Result<PathBuf> {
    if let Some(path) = output_file {
        ...
        if let (Some(file_name), Some(dir)) = (input_file_name.and_then(|s| s.to_str()), dir) {
            ...
            Err(eyre!("Couldn't create file name as input file is incomplete/invalid"))
        }
    }
}