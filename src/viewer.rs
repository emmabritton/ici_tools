...
use pixels_graphics_lib::{Options, run, System};
...
fn main() -> color_eyre::Result<()> {
    ...
    let filename = args.input.file_stem().and_then(|s| s.to_str()).unwrap_or("Image");
    ...
    run(image.width() as usize, image.height() as usize, filename, ImageDisplayer::new(image), Options::default())?;
